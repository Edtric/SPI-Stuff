...
void spiClock_Start(void) 
{
    ...
    spiClock_CLKEN |= spiClock_CLKEN_MASK;
	spiClock_CLKSTBY |= spiClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: spiClock_Stop
********************************************************************************
*
* Summary:
...
void spiClock_Stop(void) 
{
    ...
    spiClock_CLKEN &= (uint8)(~spiClock_CLKEN_MASK);
	spiClock_CLKSTBY &= (uint8)(~spiClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: spiClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((spiClock_CLKEN & spiClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = spiClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = spiClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* spiClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(spiClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(spiClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: spiClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: spiClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = spiClock_GetSourceRegister();
    uint16 oldDivider = spiClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = spiClock_CLKEN & spiClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((spiClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: spiClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(spiClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: spiClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    spiClock_MOD_SRC |= modeBitMask & (uint8)spiClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: spiClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    spiClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(spiClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: spiClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return spiClock_MOD_SRC & (uint8)(spiClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: spiClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = spiClock_GetDividerRegister();
    uint8 oldSrc = spiClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        spiClock_MOD_SRC |= CYCLK_SSS;
        spiClock_MOD_SRC =
            (spiClock_MOD_SRC & (uint8)(~spiClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: spiClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return spiClock_MOD_SRC & spiClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: spiClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    spiClock_PHASE = clkPhase & spiClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: spiClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return spiClock_PHASE & spiClock_PHASE_MASK;
}
