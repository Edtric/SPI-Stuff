...
void clock_SPI_Start(void) 
{
    ...
    clock_SPI_CLKEN |= clock_SPI_CLKEN_MASK;
	clock_SPI_CLKSTBY |= clock_SPI_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock_SPI_Stop
********************************************************************************
*
* Summary:
...
void clock_SPI_Stop(void) 
{
    ...
    clock_SPI_CLKEN &= (uint8)(~clock_SPI_CLKEN_MASK);
	clock_SPI_CLKSTBY &= (uint8)(~clock_SPI_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock_SPI_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clock_SPI_CLKEN & clock_SPI_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clock_SPI_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock_SPI_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock_SPI__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock_SPI_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock_SPI_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock_SPI_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clock_SPI_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clock_SPI_GetSourceRegister();
    uint16 oldDivider = clock_SPI_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock_SPI_CLKEN & clock_SPI_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock_SPI_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clock_SPI_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clock_SPI_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock_SPI_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_SPI_MOD_SRC |= modeBitMask & (uint8)clock_SPI_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock_SPI_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_SPI_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock_SPI_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock_SPI_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clock_SPI_MOD_SRC & (uint8)(clock_SPI_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock_SPI_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clock_SPI_GetDividerRegister();
    uint8 oldSrc = clock_SPI_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock_SPI_MOD_SRC |= CYCLK_SSS;
        clock_SPI_MOD_SRC =
            (clock_SPI_MOD_SRC & (uint8)(~clock_SPI_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock_SPI_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clock_SPI_MOD_SRC & clock_SPI_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock_SPI_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clock_SPI_PHASE = clkPhase & clock_SPI_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock_SPI_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clock_SPI_PHASE & clock_SPI_PHASE_MASK;
}
